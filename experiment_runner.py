...
def run_xor_trial(streamlining_rounds, num_vars, density, xor_num_vars=2, timeout=60):
	d = create_env()
	output = check_output('''python generate_xor_cnf.py \
		--num_variables=%d \
		--xor_density=%0.2f \
		--xor_num_vars=%d \
...
def run_sudoku_trial(streamlining_rounds, density, timeout=60):
	d = create_env()

	with cwd(d):
		sat_output = run_sat('hard_sudoku.cnf', timeout, streamlining_rounds=streamlining_rounds)
		shutil.rmtree(d)
		return sat_output
...
def main():
	...
	ap.add_argument('--sudoku', action="store_true", default=False)
	args = ap.parse_args()
	...
	for density in np.linspace(args.density_min, args.density_max, args.num_bins):
		...
		for _ in tqdm(range(args.num_trials)):
			for rounds in [0, 50, 100]:
				if args.sudoku:
					is_sat, is_contradiction = run_sudoku_trial(streamlining_rounds=rounds, density=density, timeout=args.timeout)
				else:
